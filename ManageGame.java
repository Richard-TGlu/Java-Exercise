...
public class ManageGame extends JPanel {
    ...
    public ManageGame(MainMenu mainMenu, Team team1, Team team2) {
        this.team1 = team1;
        this.team2 = team2;
        teamDAO = new TeamDAO();
        gameDAO = new GameDAO();
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row != -1) {
                        ...
                        showLoadingScreen();
                        new LoadManageGameStatsWorker(game).execute();
                    }
                }
            }
        });
    ...
    private class LoadGamesWorker extends SwingWorker<ArrayList<Game>, Void> {
        ...
        protected void done() {
            try {
                games = get();
                updateGameListPanel();
                cardLayout.show(ManageGame.this, "gameList");
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private class LoadManageGameStatsWorker extends SwingWorker<ManageGameStats, Void> {
        private Game game;

        public LoadManageGameStatsWorker(Game game){
            this.game = game;
        }
        
        @Override
        protected ManageGameStats doInBackground() throws Exception {
            return new ManageGameStats(game, ManageGame.this);
        }

        @Override
        protected void done() {
            try {
                ManageGameStats manageGameStats = get();
                add(manageGameStats, String.valueOf(game.getGameId()));
                cardLayout.show(ManageGame.this, String.valueOf(game.getGameId()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
